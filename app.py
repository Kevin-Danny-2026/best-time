...
def check_label_2(df_d, df_w, df_m):
    """
    标注2：次优形态
    月线：底背离 (新低但RSI/MACD未新低) 或 长下影止跌K线
    周线：双重底 (两相近低点+2底缩量+突破颈线)
    日线：放量突破颈线，回踩颈线不破
    """
    ...
    m_valid = (m_low_new and (m_rsi_no_new_low or m_dif_no_new_low)) or m_long_shadow
    if not m_valid:
        return False, ""
...
def check_label_4(df_d, df_w, df_m):
    """
    标注4：形态突破
    周线：上升三角形/箱体突破 (高点水平受阻，低点抬高，放量突破阻力)
    月线：突破当月量能放大 或 月线MACD零轴附近金叉
    """
...
max_threads = st.sidebar.slider("并行计算线程数", min_value=2, max_value=20, value=8)
...
if st.button("🚀 开始形态检测筛选", type="primary"):
    with st.spinner("正在获取全A股数据清单..."):
        try:
            stock_info_df = ak.stock_zh_a_spot_em()
            stock_list = list(zip(stock_info_df['代码'], stock_info_df['名称']))
            if stock_limit > 0:
                stock_list = stock_list[:stock_limit]
        except Exception as e:
            st.error(f"获取股票列表失败，错误: {e}")
            stock_list = []
